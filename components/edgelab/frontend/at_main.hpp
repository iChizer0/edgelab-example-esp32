...
void run() {
    ...
    *storage << el_make_storage_kv("boot_count", ++boot_count);
    ...
    instance->register_cmd("STAT?", "Get device status", "", el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
                               get_device_status(argv[0], boot_count);
                               return EL_OK;
                           }));
    ...
    instance->register_cmd(
      "BREAK", "Stop all running tasks", "", el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
          executor->add_task([cmd = std::string(argv[0])](std::atomic<bool>& stop_token) { break_task(cmd); });
          return EL_OK;
      }));
    ...
    instance->register_cmd(
      "MODEL", "Load a model by model ID", "MODEL_ID", el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
          ...
          executor->add_task([&, cmd = std::string(argv[0]), model_id = std::move(model_id)](
                               std::atomic<bool>& stop_token) { set_model(cmd, model_id, engine, current_model_id); });
          return EL_OK;
      }));

    instance->register_cmd("MODEL?", "Get current model info", "", el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
                               get_model_info(argv[0], engine, current_model_id);
                               return EL_OK;
                           }));
      ...
      el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
          ...
          executor->add_task(
            [&, cmd = std::string(argv[0]), algorithm_type = std::move(algorithm_type)](std::atomic<bool>& stop_token) {
                set_algorithm(cmd, algorithm_type, current_algorithm_type);
            });
          return EL_OK;
      }));
    ...
    instance->register_cmd(
      "ALGO?", "Get current algorithm info", "", el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
          get_algorithm_info(argv[0], current_algorithm_type);
          return EL_OK;
      }));
      ...
      el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
          ...
          executor->add_task(
            [&, cmd = std::string(argv[0]), sensor_id = std::move(sensor_id), enable = std::move(enable)](
              std::atomic<bool>& stop_token) { set_sensor(cmd, sensor_id, enable, current_sensor_id); });
          return EL_OK;
      }));

    instance->register_cmd(
      "SENSOR?", "Get current sensor info", "", el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
          get_sensor_info(argv[0], current_sensor_id);
          return EL_OK;
      }));
      ...
      "SAMPLE", "Sample data from current sensor", "N_TIMES", el_repl_cmd_cb_t([&](std::vector<std::string> argv) {
          ...
          executor->add_task(
            [&, cmd = std::string(argv[0]), n_times = std::move(n_times)](std::atomic<bool>& stop_token) {
                run_sample(cmd, n_times, stop_token, current_sensor_id, is_sample);
            });
          return EL_OK;
      }));
